...
void AddTestMenuItems(CefRefPtr<CefMenuModel> test_menu) {
	// Test Menu
  test_menu->AddItem(ID_TESTS_GETSOURCE, "   获取源代码  ");
...
void AddFileMenuItems(CefRefPtr<CefMenuModel> file_menu) {
  file_menu->AddItem(ID_QUIT, "E&xit"); // File Menu Items
...
void ViewsWindow::CreateMenuModel() {
  ...
  if (top_menu_bar_) {
    ...
    AddFileMenuItems(top_menu_bar_->CreateMenuModel("&文件", NULL));
    AddTestMenuItems(top_menu_bar_->CreateMenuModel("&测试", NULL));
  }
}
...
void ViewsWindow::AddControls() {
  ...
  browse_buttons.push_back(CreateBrowseButton("向后", ID_BACK_BUTTON));
  browse_buttons.push_back(CreateBrowseButton("向前", ID_FORWARD_BUTTON));
  browse_buttons.push_back(CreateBrowseButton("刷新", ID_RELOAD_BUTTON));
  browse_buttons.push_back(CreateBrowseButton("停止", ID_STOP_BUTTON));
